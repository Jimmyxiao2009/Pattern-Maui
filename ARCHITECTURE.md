...
> 版本 0.3 · 2026-07-14（同步移动端、中继、角色槽与快捷键配置实现）
...
| M3 Computer Use | ~85% | Win UIA / mac AX、视觉与无视觉无障碍树模式、T0–T3、审查窗、急停、journal；mac 真机签名/TCC 待验收 |
...
>
> **现状（0.3.0）**：已走 Node 退路（`node:sqlite` + FTS5，无 sqlite-vec）。开发依赖本机 Node 22+；单文件 sidecar 打包与 sqlite 扩展加载仍待验证。Agent 侧为自研 loop，未接 pi-mono——后续以 `packages/core` 的 `AgentCore` 边界替换，而不是绑定 pi。
...
| **M3 Computer use** | OS Bridge 工具、执行人格、安全分级、执行审查窗、急停 | "帮我把这批文件整理好"全程可视可停 |
