<script lang="ts">
  ...
  let{onComplete,mode='setup',initialPersona=null,onCancel}:{onComplete:(persona:Persona,model:ModelSetup)=>Promise<void>;mode?:'setup'|'redefine'|'new';initialPersona?:Persona|null;onCancel?:()=>void}=$props();
...
<div class="oobe-backdrop">
  <div class="oobe-window" role="dialog" aria-modal="true" aria-label={mode==='redefine'?'重新定义人格':mode==='new'?'新建人格卡':'首次启动设置'}>
    <aside class="oobe-side">
      ...
      </div>{:else}<div class="oobe-steps"><div class="current"><i>3</i><UserRound size={15}/>{mode==='new'?'新建人格卡':'重新定义人格'}</div></div>{/if}
      <p>{mode==='redefine'?'只修改人格，不会重置模型接入和系统权限。':mode==='new'?'新卡会保存到人格卡槽，并立即作为当前人格使用。':step===1?'Pattern 不托管密钥，它只会存入系统凭据管理器。':step===2?'权限随时可以在系统设置中收回。':'没有模板。你写下什么，TA 就是什么样。'}</p>
    </aside>
    <div class="oobe-content">
      <header><span>{mode==='redefine'?'人格设置':mode==='new'?'新建人格卡':'首次启动'}</span><strong>0{step} / 03</strong></header>
      {#if step===1}
